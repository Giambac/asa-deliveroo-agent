...
\paragraph{Limitations and future work.} Crate maps are now supported: crates
are sensed, block pathfinding, and a deterministic single-tile push unblocks
otherwise-unreachable targets (Section~\ref{sec:bdi}), with an optional gated
crate PDDL domain (Section~\ref{sec:pddl}); this is validated offline, but live
crate performance on the course server has not yet been re-measured, and
chained / multi-crate Sokoban planning beyond a single push remains future work.
Challenge~2 validation is one run per scenario --- encouraging,
